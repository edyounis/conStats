...
#ifndef KCONSTATS_LIB_LOCK
#define KCONSTATS_LIB_LOCK

/**
 * Main Interface Function
 *
 * kconstats_get_and_print_stats takes a sample set, calculates the statistics 
 * listed in struct stats_t, and displays the in a nicely formated way alongside
 * a scaled histogram of the data.
 */
int kconstats_get_and_print_stats ( int64_t* sample_set, uint64_t sample_size );
 ...
 */
static inline
int64_t kconstats_get_mean ( int32_t* sample_set, uint32_t sample_size )
{
 ...
 */
static inline
int64_t kconstats_get_tolerance ( int32_t* sample_set, uint32_t sample_size )
{
	register int64_t sum;
	...
	mean = kconstats_get_mean ( sample_set, sample_size );
 ...
 */
int kconstats_calculate_stats ( int32_t* sample_set, uint32_t sample_size, stats_t* stat )
{
	// Error Checking
	...
	stat->N    = sample_size;
	stat->mean = kconstats_get_mean ( sample_set, sample_size );
	stat->tolerance = kconstats_get_tolerance ( sample_set, sample_size );
 ...
 */
static inline
int32_t kconstats_zrange_value ( stats_t* stat, int64_t zScore )
{
 ...
 */
static inline
int64_t kconstats_zscore_value ( stats_t* stat, int32_t value )
{
 ...
 */
static inline
uint32_t kconstats_count_in_range ( int32_t* sample_set, uint32_t sample_size, int64_t min, int64_t max )
{
	register uint32_t i;
 ...
 */
static inline
int kconstats_print_zrange_bar ( int32_t* sample_set, uint32_t sample_size, stats_t* stat, int64_t zScoreMin, int64_t zScoreMax )
{
	char bar[33];

	bar[32] = '\0';

	int32_t value_below = kconstats_zrange_value( stat, zScoreMin );
	int32_t value_above = kconstats_zrange_value( stat, zScoreMax );

	// Calculate the number of 'X's
	uint32_t count   = kconstats_count_in_range( sample_set, sample_size, value_below, value_above );
 ...
 */
static inline
int kconstats_print_zhistogram ( int32_t* sample_set, uint32_t sample_size, stats_t* stat )
{
	int64_t zNINF = kconstats_zscore_value( stat, NINF_32 );
	int64_t zINF  = kconstats_zscore_value( stat, INF_32  );

	int64_t i = kconstats_zscore_value( stat, stat->min );
	i = i < -3 ? -3 : i; // i = max(-3, min)
	i = i < zNINF ? zNINF : i; // i = max(i, z(NINF))

	int64_t maxZ = kconstats_zscore_value( stat, stat->max );
	maxZ = maxZ > 3 ? 3 : maxZ; // maxZ = min(3, max)
	maxZ = maxZ > zINF ? zINF : maxZ; // maxZ = min(maxZ, z(INF))

	if ( i >= maxZ )
		kconstats_print_zrange_bar( sample_set, sample_size, stat, -1, 1 );

	for ( ; i < maxZ; i++ )
		kconstats_print_zrange_bar( sample_set, sample_size, stat, i, i+1 );
 ...
 */
int kconstats_print_stats ( int32_t* sample_set, uint32_t sample_size, stats_t* stat )
{
	pr_err ( "-------------------------------------------------------------------------------\n" );
	...
	kconstats_print_zhistogram( sample_set, sample_size, stat );
	pr_err ( "\n" );
 ...
 */
int kconstats_get_and_print_stats ( int32_t* sample_set, uint32_t sample_size )
{
	int error_code = 0;

	stats_t stats;
	error_code = kconstats_calculate_stats ( sample_set, sample_size, &stats );

	if ( error_code != 0 )
		return error_code;

	return kconstats_print_stats ( sample_set, sample_size, &stats );
}
